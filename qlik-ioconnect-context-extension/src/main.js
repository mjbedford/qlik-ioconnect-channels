import './qlik-ioconnect-contexts.css';
...
define(['jquery', 'qlik'], function ($, qlik) {  'use strict';
  ...
  /**
   * Publish data to an io.Connect shared context
   */
  async function publishToContext(io, contextName, data) {
    console.log(`Publishing data to context: ${contextName}`, data);
    console.log('Current io.Connect version:', io.version);
    try {
      await io.contexts.update(contextName, data);

      console.log('Data published to context:', contextName, data);
      return { success: true, context: contextName };
    } catch (error) {
      console.error('Failed to publish to context:', error);
      throw error;
    }
  }
    ...
    paint: function ($element, layout) {
      ...
      $container.find('.publish-btn').on('click', async function (e) {
        ...
        try {
          ...
          await publishToContext(ioInstance, config.contextName, payload);
